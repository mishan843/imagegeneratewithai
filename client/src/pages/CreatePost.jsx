...
const CreatePost = () => {
  ...
  const generateImgae = async() => {
      if(form.prompt){
        try {
          ...
          localStorage.setItem("Imagedata", data.photo)
  ...
  const handleSubmit = async(e) => {
    ...
    if(form.prompt && form.photo){
      setLoading(true);
      try {
        const response = await fetch('http://localhost:7000/api/v1/post', {
          method:'POST',
          headers:{
            'Content-Type':'application/json',
          },
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="mt-10">
          <p className="mt-2 text-[#666e75] text-[14px]">
            Once you have created the image you want, you can shate it with
            others in the commuinty
          </p>
          <button
            type="submit"
